...
DUMMY_PROXY_RUN=1 PROXY_TARGET_URL=http://localhost:8443 LISTEN_ADDR=":9443" TLS_CERT_PEM=./tmp/server.crt TLS_KEY_PEM=./tmp/server.key ./auth_proxy.test
...
REMOTE_CONTROL_HOME=$PWD/tmp REMOTE_CONTROL_AUTH_MODE=proxy ./remote-control --server https://localhost:9443 bash
```
